...
$form = ActiveForm::begin([
    ...
    'id' => 'slider-form',
    'validationUrl' => Url::to(['slider/validate', 'id' => $model->id]),
    ...
    <div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect js-i18n-tabs">
        ...
        <?php foreach(\Yii::$app->params['i18n'] as $key => $language): ?>
            <div class="mdl-tabs__panel <?php if($key === \Yii::$app->language): ?>is-active<?php endif; ?>" id="i18n-panel-<?= $key ?>">
                ...
                <?= $form->field($model->i18n($key), "[{$key}]caption")->textarea(['rows' => 8, 'class'=>'js-wysiwyg wysiwyg']) ?>
                <?= $form->field($model->i18n($key), "[{$key}]button_text")->textInput(['maxlength' => true]) ?>
            </div>
        <?php endforeach; ?>
    </div>
</div>
<div class="mdl-cell mdl-cell--padding mdl-cell--4-col mdl-cell--12-col-tablet mdl-shadow--2dp">
    ...
    <?= $form->field($model, 'text_position')->dropDownList($model::getTextPositions(), []) ?>
