    ...
    public function actionIndex()
    {
        ...
        if (!$xml_sitemap = Yii::$app->cache->get('sitemap')) {  // проверяем есть ли закэшированная версия sitemap
			...
			foreach ($items as $item) {
                $urls[] = array(
                    htmlspecialchars(Url::to($item->getUrl(),true)), 'daily'   
                );
            }
            ...
            foreach (array_keys(ProductType::getTypesAsArray()) as $key) {    
                ...
                $urls[] = array(
                            htmlspecialchars(Url::home(true).'brand/'.$key), 'daily'   
                        );
                ...
                foreach ($makers as $maker) {
                    $urls[] = array(
                        htmlspecialchars(Url::home(true).'brand/'.$key.'/'.$maker['name']), 'daily'   
                    );
                    ...
                    foreach ($models as $modelauto) {
                        $urls[] = array(
                            htmlspecialchars(Url::home(true).'brand/'.$key.'/'.$maker['name'].'/'.$modelauto['id']), 'daily'   
                        );
                    }
                }
            }
